...
function toSubcommand(canonical: string, family: 'state' | 'verify' | 'init' | 'phase' | 'phases' | 'validate' | 'roadmap' | 'gitnexus'): string {
  const prefix = `${family}.`;
  return canonical.startsWith(prefix) ? canonical.slice(prefix.length) : canonical;
}

async function main(): Promise<void> {
  const stateEntries = COMMAND_DEFINITIONS_BY_FAMILY.state.map((entry) => ({ canonical: entry.canonical, aliases: entry.aliases, subcommand: toSubcommand(entry.canonical, 'state'), mutation: entry.mutation }));
  const verifyEntries = COMMAND_DEFINITIONS_BY_FAMILY.verify.map((entry) => ({ canonical: entry.canonical, aliases: entry.aliases, subcommand: toSubcommand(entry.canonical, 'verify'), mutation: entry.mutation }));
  const initEntries = COMMAND_DEFINITIONS_BY_FAMILY.init.map((entry) => ({ canonical: entry.canonical, aliases: entry.aliases, subcommand: toSubcommand(entry.canonical, 'init'), mutation: entry.mutation }));
  const phaseEntries = COMMAND_DEFINITIONS_BY_FAMILY.phase.map((entry) => ({ canonical: entry.canonical, aliases: entry.aliases, subcommand: toSubcommand(entry.canonical, 'phase'), mutation: entry.mutation }));
  const phasesEntries = COMMAND_DEFINITIONS_BY_FAMILY.phases.map((entry) => ({ canonical: entry.canonical, aliases: entry.aliases, subcommand: toSubcommand(entry.canonical, 'phases'), mutation: entry.mutation }));
  const validateEntries = COMMAND_DEFINITIONS_BY_FAMILY.validate.map((entry) => ({ canonical: entry.canonical, aliases: entry.aliases, subcommand: toSubcommand(entry.canonical, 'validate'), mutation: entry.mutation }));
  const roadmapEntries = COMMAND_DEFINITIONS_BY_FAMILY.roadmap.map((entry) => ({ canonical: entry.canonical, aliases: entry.aliases, subcommand: toSubcommand(entry.canonical, 'roadmap'), mutation: entry.mutation }));
  const gitnexusEntries = COMMAND_DEFINITIONS_BY_FAMILY.gitnexus.map((entry) => ({ canonical: entry.canonical, aliases: entry.aliases, subcommand: toSubcommand(entry.canonical, 'gitnexus'), mutation: entry.mutation }));

  const nonFamilyEntries = [...NON_FAMILY_COMMAND_MANIFEST]
    .sort((a, b) => a.canonical.localeCompare(b.canonical))
    .map((entry) => ({ canonical: entry.canonical, aliases: [...entry.aliases], mutation: entry.mutation }));
  ...
  const tsHeader = `/**\n * GENERATED FILE - command alias expansion for state.*, verify.*, init.*, phase.*, phases.*, validate.*, roadmap.*, gitnexus.*, and non-family commands.\n * Source: sdk/src/query/command-manifest.{state,verify,init,phase,phases,validate,roadmap,gitnexus,non-family}.ts\n */\n\n`;
  const tsBody = [
    'export interface FamilyCommandAlias {', '  canonical: string;', '  aliases: string[];', '  subcommand: string;', '  mutation: boolean;', '}', '',
    ...
    `export const GITNEXUS_COMMAND_ALIASES: readonly FamilyCommandAlias[] = ${renderFamilyArray(gitnexusEntries)} as const;`, '',
    'export interface NonFamilyCommandAlias {', '  canonical: string;', '  aliases: string[];', '  mutation: boolean;', '}', '',
    ...
    'export const GITNEXUS_SUBCOMMANDS = new Set<string>(GITNEXUS_COMMAND_ALIASES.map((entry) => entry.subcommand));', '',
  ].join('\n');
  ...
  const cjsBody = [
    ...
    `const GITNEXUS_COMMAND_ALIASES = ${JSON.stringify(gitnexusEntries, null, 2)};`, '',
    ...
    `module.exports = {`,
    `  STATE_COMMAND_ALIASES,`, `  VERIFY_COMMAND_ALIASES,`, `  INIT_COMMAND_ALIASES,`, `  PHASE_COMMAND_ALIASES,`, `  PHASES_COMMAND_ALIASES,`,
    `  VALIDATE_COMMAND_ALIASES,`, `  ROADMAP_COMMAND_ALIASES,`, `  GITNEXUS_COMMAND_ALIASES,`, `  NON_FAMILY_COMMAND_ALIASES,`,
    `  STATE_SUBCOMMANDS,`, `  VERIFY_SUBCOMMANDS,`, `  INIT_SUBCOMMANDS,`, `  PHASE_SUBCOMMANDS,`, `  PHASES_SUBCOMMANDS,`,
    `  VALIDATE_SUBCOMMANDS,`, `  ROADMAP_SUBCOMMANDS,`, `  GITNEXUS_SUBCOMMANDS,`, `};`,
  ].join('\n');
